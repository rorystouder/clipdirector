...
### **Phase 10 — Full Android Implementation (Amendment 2026-05-19)**

| ℹ️ CONTEXT Phase 9 verified the scaffold compiles. Phase 10 turns it into a working app — registration / login against the gateway, video picking via the system PhotoPicker, multipart upload with progress, status polling, ExoPlayer playback of the rendered MP4, and a history list of past jobs. Same-day extension of Phase 9: while Phase 9 was running its assembleDebug, the user picked Option B (full implementation) and authorized the larger scope. This phase produces a debuggable APK an end user can run on an Android 13+ emulator (or real device via `adb reverse`) and exercise the full flow against `docker compose up`. Architectural decisions (DI strategy, auth refresh pattern, persistence choice, file picker, player) were all locked in after a Plan-agent critique — see `/home/rorystouder/.claude/plans/typed-churning-harp.md` for the rationale on each, and the chunk-level commits (10.1 through 10.7) for the actual implementation. **OUT OF SCOPE for Phase 10 (deferred to Phase 11):** WorkManager + foreground service for upload resilience under process death, push notifications, multi-account, tablet layouts, Crashlytics, R8 hardening, localization. |
| :---- |

67. Phase 10.1 — Foundation: add `kotlin.plugin.serialization` Gradle plugin in root build, apply it in `app/build.gradle.kts` and add deps `androidx.datastore:datastore-preferences`, `androidx.security:security-crypto`, `lifecycle-viewmodel-ktx`, `lifecycle-runtime-compose`, `coil-video`, `kotlinx-coroutines-android`, MockWebServer/MockK/Turbine (test). Add `buildTypes { debug { ... }; release { ... } }` with `BuildConfig.API_BASE_URL` (debug = `http://10.0.2.2:3000/`, release env-overridable). Update `AndroidManifest.xml` with `android:name=".ClipDirectorApp"`, `READ_MEDIA_VIDEO`, `READ_EXTERNAL_STORAGE` (maxSdkVersion 32), `networkSecurityConfig` reference. Create `res/xml/network_security_config.xml` allowing cleartext for `10.0.2.2` / `localhost` only. Create stub `ClipDirectorApp.kt`

68. Phase 10.2 — Data layer: hand-rolled DI via `AppContainer` + `ServiceLocator` (no Hilt). `TokenStore` (DataStore-backed, exposes `Flow<Tokens?>`). `AuthApi` (public — register / login / refresh, on unauthed OkHttp client to avoid recursive refresh). `AccountApi` (Bearer — logout / me, on authed client). `AuthInterceptor` (synchronously reads `TokenStore` via `runBlocking`, adds Bearer). `AuthAuthenticator` (OkHttp `Authenticator`, NOT Interceptor — single-flight `Mutex` around `/auth/refresh`, hard-cap at 2 retries to prevent loops). `JobApi` (typed `@SerialName` enums for `JobStatus` / `Platform` / `MusicMood` / `CaptionStyle`). `JobIdStore` (DataStore CSV of jobIds, cap 100). `CountingRequestBody` (wraps RequestBody, emits progress to a callback throttled to every 64 KB). `JobRepository.submitJob` uses `channelFlow` not `flow` so the off-thread upload callback can `trySend` progress without violating flow's single-emission invariant; `AtomicLong` tracks per-part progress monotonically across multipart boundaries. `ErrorBus` (`SharedFlow<String>` with `DROP_OLDEST`). `ApiErrorAdapter` parses `{code, message, details?}` envelope with special cases for `rate_limited` (reads `Retry-After`), `conflict`, `validation_error`. `NetworkModule` builds two `OkHttpClient`s with the split described above. `AppContainer` + `ClipDirectorApp` wire it all. Removes the old `api/ClipDirectorApi.kt`; types live under `data/auth/` and `data/job/`

69. Phase 10.3 — Auth screens: `LoginScreen` + `LoginViewModel` (email/password, `State { Idle, Submitting, Error, Success }` as `StateFlow`, double-tap-blocked). `RegisterScreen` + `RegisterViewModel` (adds confirm-password match, email regex, password ≥ 12-char client-side validation matching gateway). Rewrite `MainActivity` to read tokens via `runBlocking { appContainer.tokenStore.current() }` at `onCreate` and choose `startDestination = "login"` vs `"clips"`. Top-level `Scaffold` + `SnackbarHostState` collecting from `ErrorBus.messages`. ViewModels constructed via `viewModelFactory { initializer { ... } }` from `LocalContext.appContainer` extension — no Hilt-style annotations

70. Phase 10.4 — Job-flow screens: rewrite five Phase-0 stubs into full implementations. `data/job/SubmissionDraft` (process-scoped `MutableStateFlow<List<Uri>>` for ClipSelect → Prompt handoff — URIs aren't trivially nav-arg-serializable). `ClipSelectScreen` + VM uses `ActivityResultContracts.PickMultipleVisualMedia(max=12)`, `MediaMetadataRetriever` (off-main via injected `ioDispatcher` for testability), Coil + `VideoFrameDecoder` for thumbnails. `PromptScreen` + VM uses `ExposedDropdownMenuBox` (Material3) for typed enum pickers, collects `JobRepository.submitJob()` `channelFlow` and renders Preparing / Uploading(fraction) / Submitted / Error states. `ProcessingScreen` + VM polls every 2 s in a `LaunchedEffect(jobId)`, short-circuits on `status.isTerminal`. `PreviewScreen` + VM uses `DisposableEffect` for `ExoPlayer` lifecycle (NOT hoisted into VM — ExoPlayer holds Activity context), `LifecycleEventObserver` pauses on `ON_STOP`, `AndroidView { PlayerView }`. `HistoryScreen` + VM collects `JobIdStore.jobIds`, hydrates each row's status on demand from the gateway. `MainActivity` gains `processing/{jobId}` and `preview/{jobId}` parameterized routes

71. Phase 10.5 — Tests (20 unit tests, all green, written failure-shaped per project rule). `MainDispatcherRule` (sets `Dispatchers.Main` to `StandardTestDispatcher`). `ApiErrorAdapterTest` (6: validation envelope, rate-limit with/without `Retry-After`, IOException, unknown exception). `AuthRepositoryTest` (5, MockWebServer: happy login + lowercased-email request body, 401, 429, conflict 409, logout-clears-tokens-on-network-failure). `LoginViewModelTest` (3: success state machine, error-then-acknowledge, double-tap dedup). `ProcessingViewModelTest` (3, Turbine + virtual time: COMPLETE immediate-exit, FAILED surfaces exact `errorMessage`, multi-poll progression). `ClipSelectViewModelTest` (3: empty rejection, >MAX rejection, valid selection populates draft). Skips instrumented UI test (deferred — needs connected device, lower-value than the unit coverage)

72. Phase 10.6 — Polish: `./gradlew :app:assembleDebug` clean (verified end of each chunk). `./gradlew :app:lintDebug` clean. `apps/android/README.md` rewrite from Phase-0/9 "scaffold only" framing to a Phase-10 run-on-device walkthrough (emulator uses `10.0.2.2`; real device needs `adb reverse tcp:3000 tcp:3000`). Manual smoke test by the user against `docker compose up` (dev/MinIO mode) is the Phase 10 close gate

73. Phase 10.7 — Docs: this PRD amendment + Phase 10 entry in `Docs/project_completion_summary.md`

74. Verify: `./gradlew :app:testDebugUnitTest` reports 20+ tests passing

75. Verify: `./gradlew :app:assembleDebug` exits 0 and produces `app/build/outputs/apk/debug/app-debug.apk`

76. Verify (user-gated): install the APK on an emulator or device with `adb reverse` in place, run through the full happy path (register → pick clips → submit → poll → preview → share → history persists across force-stop)
